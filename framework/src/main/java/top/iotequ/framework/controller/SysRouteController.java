...
@RestController
@RequestMapping("/framework/sysRoute")
public class SysRouteController  {
	private static final Logger log = LoggerFactory.getLogger(SysRouteController.class);
	@Autowired
	ICgService<SysRoute> cgService;
	...
	public ResponseEntity<Map<String, Object>> defaultValue(Boolean needLoadDictionary, String requestDynaFields, SysRoute sysRoute,HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			if (!Util.isEmpty(requestDynaFields)) return new RestJson().dictionary(cgService.getDictionary(sysRoute,true, requestDynaFields)).toResponse();
			else {
				sysRoute = cgService.getDefaultObject(sysRoute);
                return new RestJson().data(sysRoute)
                    .dictionary(Objects.nonNull(needLoadDictionary) && needLoadDictionary ? cgService.getDictionary(sysRoute,true,null) : null).toResponse();
			}
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
		}
	...
	public ResponseEntity<Map<String, Object>> record(Boolean needLoadDictionary, Boolean loadDictionaryOnly, String requestDynaFields, String id, SysRoute sysRoute, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			RestJson j = new RestJson();
			if (!Util.isEmpty(id)) sysRoute=cgService.getRecord(id,request);
			if (!Util.isEmpty(requestDynaFields) || (loadDictionaryOnly!=null && loadDictionaryOnly)) {
				return j.dictionary(cgService.getDictionary(sysRoute,true, requestDynaFields)).toResponse();
			}
			if (Objects.nonNull(needLoadDictionary) && needLoadDictionary) j.dictionary(cgService.getDictionary(sysRoute,true,null));
			j.data(sysRoute);
			return j.toResponse();
		} catch (Exception e) {
			return new RestJson().setError(e).toResponse();
	...
	public ResponseEntity<Map<String, Object>> save(String flowCode, Integer total_filepart, SysRoute sysRoute, String idSaved, HttpServletRequest request, HttpServletResponse response) {
		try {
			boolean isNew = (Util.isEmpty(idSaved) || Util.isEmpty(sysRoute.getId()));
			return 	cgService.doSave(isNew, flowCode,total_filepart, sysRoute, idSaved, request).toResponse();
		} catch (Exception e) {
	...
	public ResponseEntity<Map<String, Object>> export(String sort,String order, String search,HttpServletRequest request, HttpServletResponse response){
		try {
			cgService.export(null,0,100, null, "sysRoute.xlsx", "sysRoute", sort, order, search, request, response);
			return null;
		} catch (Exception e) {
			response.reset();
